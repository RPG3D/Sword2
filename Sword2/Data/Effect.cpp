...
{	
	...
	int tempX = fmaxf((int)(((double)flyingDirection.x) / l * width * TILE_HEIGHT / 2) - 1, 1);
	int tempY = fmaxf((int)(((double)flyingDirection.y) / l * width * TILE_WIDTH / 2) - 1, 1);
	...
	auto maxStep = fmaxf(fmaxf(tempPath[0].size(), tempPath[1].size()), tempPath[2].size());
